  ...
  <div class="mx-auto max-w-[1472px] overflow-x-hidden px-0 pb-24 pt-2 sm:px-6 xl:pb-8 lg:px-8">
    <div class="mb-2 flex items-start justify-between gap-3 px-3 sm:mb-4 sm:px-0">
      ...
      <div class="flex shrink-0 items-center gap-2">
        <router-link
          to="/"
          class="inline-flex h-9 items-center justify-center rounded-full border border-gray-200 bg-white px-3 text-sm font-semibold text-gray-700 transition hover:border-orange-500 hover:text-orange-600 dark:border-white/10 dark:bg-neutral-900 dark:text-gray-200 sm:h-10 sm:px-4"
        >
          {{ t('cart.emptyAction') }}
        </router-link>
        <button
    ...
    >
      <div class="flex min-h-[260px] flex-col items-center justify-center px-6 py-12 text-center">
        <div class="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-orange-50 text-orange-600 dark:bg-orange-500/10">
        ...
        <router-link
          to="/"
          class="mt-5 inline-flex h-10 items-center justify-center rounded-full bg-[#ff5000] px-6 text-sm font-bold text-white hover:bg-orange-600"
        >
          {{ t('cart.emptyAction') }}
        </router-link>
      </div>
      ...
      <section class="overflow-hidden border-y border-gray-100 bg-white/95 shadow-[0_8px_28px_rgba(15,23,42,0.04)] ring-0 dark:border-white/10 dark:bg-neutral-900/95 sm:rounded-2xl sm:border-0 sm:shadow-sm sm:ring-1 sm:ring-gray-100 sm:dark:ring-white/10">
        <div class="flex items-center justify-between gap-3 border-b border-gray-100 px-3 py-2 dark:border-white/10 lg:hidden">
          <div class="flex items-center gap-2">
            ...
            <span class="rounded-full bg-orange-50 px-2 py-0.5 text-xs font-bold text-[#ff5000] dark:bg-orange-500/10">
        ...
        <div class="hidden items-center justify-between gap-3 px-4 py-4 sm:px-5 lg:flex">
          <div class="flex items-center gap-2">
            ...
            <span class="rounded-full bg-orange-50 px-2.5 py-1 text-xs font-bold text-[#ff5000] dark:bg-orange-500/10">
              {{ cartStore.totalItems }}
            </span>
          </div>
          ...
          >
            ...
            <div class="grid min-w-0 grid-cols-[56px_minmax(0,1fr)] gap-2.5">
              ...
              <div class="min-w-0 pr-7">
                <button
                  type="button"
                  class="line-clamp-1 text-left text-sm font-semibold leading-5 text-gray-950 [overflow-wrap:anywhere] hover:text-orange-600 dark:text-white"
                  @click="goProduct(item)"
                >
                ...
                <div class="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px]">
                  <span class="rounded-full bg-orange-50 px-2 py-0.5 font-semibold text-[#ff5000] dark:bg-orange-500/10">
                ...
                <div class="mt-1.5 flex items-center justify-between gap-2 rounded-xl bg-[#f8f5f0] px-2 py-1.5 ring-1 ring-black/[0.03] dark:bg-white/[0.04] dark:ring-white/10">
                  <div class="min-w-0 text-base font-black leading-none text-[#ff5000]">{{ itemSubtotal(item) }}</div>
                  <MallQuantityInput
          ...
          >
            <div class="grid min-w-0 grid-cols-[76px_minmax(0,1fr)] gap-3">
              ...
              <div class="min-w-0">
                <button
                  type="button"
                  class="line-clamp-2 text-left text-sm font-semibold leading-6 text-gray-950 [overflow-wrap:anywhere] hover:text-orange-600 dark:text-white"
                  @click="goProduct(item)"
                >
                  {{ titleText(item.title) }}
                </button>
            ...
            <div class="flex min-h-[72px] items-center justify-center">
              <div class="text-center text-lg font-black text-[#ff5000]">{{ itemSubtotal(item) }}</div>
            </div>
      ...
      <aside class="hidden rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100 dark:bg-neutral-900/95 dark:ring-white/10 xl:sticky xl:top-4 xl:block">
        ...
        <div class="mt-5 space-y-3 text-sm">
          ...
          <div class="flex items-center justify-between border-t border-gray-100 pt-3 dark:border-white/10">
            ...
            <span class="text-xl font-black text-[#ff5000]">{{ moneyText(totalAmount) }}</span>
          </div>
        </div>
      ...
      <div class="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 bg-white px-3 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] shadow-[0_-6px_24px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-neutral-950 xl:hidden">
        <div class="mx-auto flex max-w-[560px] items-center gap-3">
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-0.5 truncate text-lg font-black text-[#ff5000]">{{ moneyText(totalAmount) }}</div>
          </div>
          <router-link
            to="/checkout"
            class="inline-flex h-11 shrink-0 items-center justify-center rounded-xl bg-[#ff5000] px-6 text-sm font-black text-white transition hover:bg-orange-600"
          >
            {{ t('cart.checkout') }}
          </router-link>
        </div>
