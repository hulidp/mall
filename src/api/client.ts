...
const getStoredUserToken = () => localStorage.getItem('user_token') || sessionStorage.getItem('user_token')
const mallOpenAuthModalEvent = 'dujiao.mall.open-auth-modal'
const mallAuthRedirectStorageKey = 'dujiao.mall.authRedirect'
...
const requestUserLogin = () => {
    clearStoredUserAuth()
    if (typeof window === 'undefined') return
    window.dispatchEvent(new CustomEvent(userAuthInvalidEvent))

    const redirect = currentRedirectPath()
    if (redirect.startsWith('/')) {
        window.sessionStorage.setItem(mallAuthRedirectStorageKey, redirect)
    }

    const handledByTemplate = !window.dispatchEvent(new CustomEvent(mallOpenAuthModalEvent, {
        cancelable: true,
        detail: {
            mode: 'login',
            redirect,
    ...
    if (!handledByTemplate) {
        const query = redirect && redirect !== '/auth/login'
