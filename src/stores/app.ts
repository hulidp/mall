...
export const useAppStore = defineStore('app', () => {
    ...
    const normalizeLocale = (value?: string | null) => {
        return value && supportedLocales.includes(value) ? value : 'zh-CN'
    }
    const syncI18nLocale = (value: string) => {
        const globalLocale = i18n.global.locale
        if (typeof globalLocale === 'string') {
            i18n.global.locale = value as any
            return
        }
        globalLocale.value = value as any
    }
    const initialLocale = normalizeLocale(localStorage.getItem('locale') || String(i18n.global.locale.value || 'zh-CN'))
    ...
    const applyDocumentLanguage = (value: string) => {
        ...
        document.documentElement.lang = value || 'zh-CN'
    }
