...
export const useUserProfileStore = defineStore('user-profile', () => {
    ...
    const loginLogsPagination = ref<PaginationState>({ page: 1, page_size: 5, total: 0, total_page: 1 })
    ...
    const loadRecentLoginLogs = async (options: number | { page?: number; pageSize?: number; page_size?: number } = 5) => {
        loadingLoginLogs.value = true
        const page = typeof options === 'number' ? 1 : Math.max(1, Number(options.page || 1))
        const pageSize = typeof options === 'number'
            ? Math.max(1, Number(options || 5))
            : Math.max(1, Number(options.pageSize || options.page_size || loginLogsPagination.value.page_size || 5))
        try {
            const response = await userProfileAPI.loginLogs({ page, page_size: pageSize })
            const data = response.data.data
            recentLoginLogs.value = Array.isArray(data) ? (data as UserLoginLogItem[]) : []
            loginLogsPagination.value = response.data.pagination || {
                page,
                page_size: pageSize,
                total: recentLoginLogs.value.length,
                total_page: 1,
            }
            return true
        } catch {
            recentLoginLogs.value = []
            loginLogsPagination.value = { page, page_size: pageSize, total: 0, total_page: 1 }
            return false
        } finally {
